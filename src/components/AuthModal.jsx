...
function AuthModal({ isOpen, onClose, onAuthSuccess, defaultTab }) {
  ...
  const [formData, setFormData] = useState({ username: '', password: '', full_name: '', email: '', newPassword: '' });
  ...
  useEffect(() => {
    ...
    setError('');
    setFormData({ username: '', password: '', full_name: '', email: '', newPassword: '' });
  }, [defaultTab, isOpen]);

  useEffect(() => {
    const saved = localStorage.getItem('wanderly_remember');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        setFormData(prev => ({ ...prev, username: parsed.username, password: parsed.password }));
        setRememberMe(true);
      } catch (e) {}
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isForgotPassword) {
      try {
        const data = await callBackend('/api/reset-password', { username: formData.username, email: formData.email, newPassword: formData.newPassword });
        if (data.status === 'success') {
    ...
    try {
      ...
      if (data.status === 'success') {
        if (isLogin) {
          if (rememberMe) {
            localStorage.setItem('wanderly_remember', JSON.stringify({ username: formData.username, password: formData.password }));
          } else {
            localStorage.removeItem('wanderly_remember');
          }
        }
        localStorage.setItem('user', JSON.stringify(data.user));
        localStorage.setItem('token', data.token);
        ...
        <div className="flex-1 flex flex-col p-8 lg:p-10 relative bg-white overflow-y-auto max-h-[90vh]">
          ...
          <div className="mb-5">
            ...
            <p className="text-gray-400 text-xs mt-1 tracking-[0.15em] uppercase font-medium">
              WanderlyVietNam · {isForgotPassword ? 'Đặt lại mật khẩu của bạn' : isLogin ? 'Đăng nhập để tiếp tục' : 'Miễn phí · Mãi mãi'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {isLogin && (
                    <button 
                      ...
                      className="text-xs font-bold text-[#0A241A]/60 hover:text-[#D4AF37] hover:underline underline-offset-2 transition-colors"
                    >
                      Quên mật khẩu?
                    </button>
                  )}
            ...
            >
              {loading ? '⏳ Đang xử lý...' : isForgotPassword ? 'Đổi Mật Khẩu' : (isLogin ? '→ Đăng Nhập' : '✦ Tạo Tài Khoản')}
            </button>
          </form>
